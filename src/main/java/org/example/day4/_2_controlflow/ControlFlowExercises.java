...
public class ControlFlowExercises {
    ...
    boolean kidsRide(int temp, boolean isWinter) {
        if (isWinter == false && temp >= 15 && 30 >= temp) {
            return true;
        } else if (isWinter && temp >= 10 && 30 >= temp) {
            return true;
        } else {
            return false;
        }
    ...
    boolean goOut(boolean isSunday, boolean isMom, boolean isPast20) {
        if (isSunday && isMom && isPast20 == false) {
            return true;
        } else if (isSunday == false && isMom == false && isPast20 == false){
            return true;
        }else {
            return false;
        }
